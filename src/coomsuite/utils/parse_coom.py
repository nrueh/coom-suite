...
class ASPModelVisitor(ModelVisitor):
    ...
    def visitOption(self, ctx: ModelParser.OptionContext):
        ...
        if constant != []:
            ...
            for a, c in zip(parent_attr, constant):
                ...
                self.output_asp.append(f'attribute_value("{parent_name}","{option_name}","{attr_name}",{option_value})')
    ...
    def visitExists(self, ctx: ModelParser.ExistsContext):
        ...
        self.output_asp.append(f'exists({self.constraint_idx},{path},"{value}")')
        self.output_asp.append(f'constant("{value}")')
